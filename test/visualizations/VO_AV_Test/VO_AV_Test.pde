...
int PHASE_VIS_ONLY = 0;
int PHASE_AV       = 1;

int phase = PHASE_VIS_ONLY;
...
void setup() {
  ...
  uiFont = createFont("Arial", int(TEXT_LARGE));
  textFont(uiFont);
}
...
int getCurrentExcerptIndex() {
  if (phase == PHASE_VIS_ONLY) return orderVis[orderPos];
  else return orderAV[orderPos];
}
